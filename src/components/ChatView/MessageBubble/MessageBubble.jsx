...
export default function MessageBubble({
  ...
  const replyMsg = msg.replyTo
    ? window.seedMessages?.find((m) => m.id === msg.replyTo)
    : null;
  ...
  const handleCtx = (e) => {
    e.preventDefault();
    setShowCtx({ x: e.clientX, y: e.clientY });
  };
            ...
            <>
              {replyMsg && (
                <div
                  ...
                  onClick={() => onReply && onReply(replyMsg)}
                >
                  <div className={styles["reply-name"]}>
                    {replyMsg.senderId === CURRENT_USER_ID
                      ? "Вы"
                      : contact.name}
                  </div>
              ...
              <span className={styles["message-meta"]}>
                {msg.isEdited && (
                  <span className={styles["edited-label"]}>ред.</span>
                )}
                <span className={styles["message-time"]}>{timeStr}</span>
                {isOutgoing && (
                  <span
                    className={`${styles["message-status"]} ${msg.isRead ? styles.read : ""}`}
                  >
                    {msg.status === "sent" ? (
              ...
              {msgReactions.map((r, i) => (
                ...
                >
                  ...
                  <span className={styles.count}>
                    {msgReactions.filter((x) => x.emoji === r.emoji).length}
                  </span>
                </span>
              ))}
            </div>
      ...
      {showCtx && (
        <>
          <div
            ...
            style={{
              top: showCtx.y,
              left: Math.min(showCtx.x, window.innerWidth - 200),
            }}
          >
            ...
            <div
              className={`${styles["ctx-item"]} ${styles.danger}`}
              onClick={() => {
